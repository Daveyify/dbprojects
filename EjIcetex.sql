...
SELECT * FROM student ORDER BY student.name ASC; -- Cuando no se especifica, se organiza de forma ascendente.
SELECT * FROM student ORDER BY cedula DESC;


-- Join sentence 
SELECT student.name, debt.student_cedula, debt.total_debt, debt.date --Le digo los datos que quiero ver en la tablka
FROM debt -- Le indico de donde va a sacar esos datos 
INNER JOIN student ON debt.student_cedula=student.cedula
/* Le indico donde esta la información que falta y le digo cual es el dato en el que se unen las dos tablas 
para que lo cree*/
